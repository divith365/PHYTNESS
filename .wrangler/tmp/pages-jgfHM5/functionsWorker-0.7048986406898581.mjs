...
async function onRequestPost({ request, env }) {
  try {
    ...
    if (!payload.pin) payload.pin = "1111";
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: errData.message || "Failed to add doctor" }), { status: response.status });
    }
    const data = await response.json();
    return new Response(JSON.stringify({ success: true, doctor: data[0] }), {
      status: 201,
...
// ../.wrangler/tmp/pages-jgfHM5/functionsRoutes-0.36649334665800304.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-Nzhgyb/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-Nzhgyb/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7048986406898581.mjs.map
